...
function WebSocketConnection(socket, extensions, protocol, maskOutgoingPackets, config) {
    ...
    if (this.config && this.config.keepaliveInterval) {
        ...
        case 0x02: // WebSocketFrame.BINARY_FRAME
            ...
            if (this.assembleFragments) {
                if (frame.fin) {
                    ...
                    // Fix for slow device's inbound message queue (time to process)
                    // exceeding keepalivetime timeout and therefore resulting in
                    // connection being dropped because ping is stuck at back of queue
                    // and so no pong arrives in time.
                    this.pong(new Buffer(0));
                }
                else {
